import { expect, test } from "../../utils/setupScriptComponent";
...
test.describe("A single WebSocket provider should be initialized per notebook", () => {
  ...
  test("Clients connecting to the same notebook should be connected to the same WebSocket Provider", async ({
    ...
    const providerIndex1 = consoleMessages1.findIndex((msg) =>
      msg.text().includes("HocuspocusProvider")
    );
    const providerIndex2 = consoleMessages2.findIndex((msg) =>
      msg.text().includes("HocuspocusProvider")
    );
